...
def main():
    ...
    all_keys = set(lord_castles)
    for e in lords:
        if e['c'] == 'person' and e['n'].endswith('家') and e['n'][:-1] + '氏' in all_keys:
            e['c'] = 'org'
...
<footer>
  ...
  <ul>
    ...
    <li>ただし「〜家」は、語幹＋「氏」が別項目として実在するもの（「森家」に対する「森氏」、「大給松平家」に対する「大給松平氏」など29件）だけ家名とみなして組織に入れている。</li>
